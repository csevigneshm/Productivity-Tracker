...
function StatCard({
    ...
    return (
        <div
            ...
            onMouseEnter={() =>
                breakdown && breakdown.length > 0 && setHovered(true)
            }
            onMouseLeave={() => setHovered(false)}
        >
            <div
                className={`absolute -top-4 -right-4 h-20 w-20 rounded-full opacity-10 blur-2xl transition-opacity duration-300 group-hover:opacity-20 ${gradient} overflow-hidden`}
            />
            <div className="relative flex items-start justify-between">
                ...
                <div
                    className={`flex h-10 w-10 items-center justify-center rounded-xl ${gradient} shadow-lg`}
                >
...
function ProfileCheckItem({
    ...
    const StatusIcon = isRange ? null : checked ? (
    ...
    return (
        <div
            ...
            ref={ref}
            onMouseEnter={() =>
                isRange && missedDates.length > 0 && setHovered(true)
            }
            onMouseLeave={() => setHovered(false)}
        >
            <div
                className={`flex items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-medium transition-all duration-200 ${bgClass}`}
            >
                {StatusIcon}
            ...
            {hovered && missedDates.length > 0 && (
                <div className="absolute bottom-full left-0 z-20 mb-2 min-w-[140px] rounded-xl border border-neutral-200 bg-white p-3 shadow-lg dark:border-neutral-700 dark:bg-neutral-900">
                    ...
                    <ul className="flex max-h-36 flex-col gap-1 overflow-y-auto">
                        {missedDates.map((d) => (
                            <li key={d} className="text-xs text-rose-500">
                                {fmtDisplay(d)}
                            </li>
                        ))}
                    </ul>
        ...
        <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 rounded-xl border border-neutral-200 bg-white px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900">
                <input
                    ...
                    className="w-full [appearance:textfield] bg-transparent text-sm text-neutral-700 focus:outline-none dark:text-neutral-200 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                />
...
export default function Dashboard({
    ...
    const appStats = useMemo<ApplicationStats>(() => {
        let data = [...allApps];
        ...
        if (appFilterMode !== 'all') {
            ...
            data = data.filter(
                (a) => a.applied_date >= from && a.applied_date <= to,
            );
        }
    ...
    const fetchLogs = () => {
        ...
        if (filterMode === 'today') {
            url = `/api/daily-logs/${todayStrVal}`;
        }

        if (filterMode === 'this-week') {
            url = `/api/daily-logs/range?from=${weekStart()}&to=${weekEnd()}`;
        }

        if (filterMode === 'this-month') {
            url = `/api/daily-logs/range?from=${monthStart()}&to=${monthEnd()}`;
        }

        if (filterMode === 'single') {
            url = `/api/daily-logs/${singleDate}`;
        }

        if (filterMode === 'range') {
            url = `/api/daily-logs/range?from=${fromDate}&to=${toDate}`;
        }
            ...
            .then((data) => {
                if (data) {
                    setForm({
                        ...
                        linkedin_applications: String(
                            data.linkedin_applications ?? '0',
                        ),
                        naukri_applications: String(
                            data.naukri_applications ?? '0',
                        ),
                        indeed_applications: String(
                            data.indeed_applications ?? '0',
                        ),
    ...
    const handleSave = async () => {
        ...
        try {
            const payload = {
                ...
                linkedin_applications:
                    parseInt(form.linkedin_applications) || 0,
    ...
    const pastDatesInRange = (() => {
        ...
        const end = new Date(
            Math.min(
                ...
                new Date(todayStrVal + 'T00:00:00').getTime(),
            ),
        );

        while (cur <= end) {
            dates.push(dateObjToStr(cur));
    ...
    const profileStats = profileFields.map(({ key, label, icon }) => {
        ...
        return {
            ...
            label,
            icon,
            count: updatedDates.size,
            total: pastDatesInRange.length,
            missedDates,
        };
    });
            ...
            <div className="flex h-full flex-1 flex-col gap-8 p-4 sm:p-6">
                        ...
                        <div className="w-full max-w-lg rounded-2xl border border-neutral-200 bg-white p-6 shadow-xl dark:border-neutral-700 dark:bg-neutral-900">
                            ...
                            <div className="mb-5 flex flex-col gap-1">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm text-neutral-700 focus:ring-2 focus:ring-neutral-900/20 focus:outline-none dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-200"
                                />
                                {formDate !== todayStrVal && (
                                    <p className="text-xs text-amber-500">
                                        Editing past date: {formDate}
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="mb-5 grid grid-cols-2 gap-4">
                                <NumInput
                                    ...
                                    onChange={f('study_hours')}
                                />
                                <NumInput
                                    label="Interview Calls"
                                    value={form.interview_calls}
                                    onChange={f('interview_calls')}
                                />
                                <NumInput
                                    label="LinkedIn Applications"
                                    value={form.linkedin_applications}
                                    onChange={f('linkedin_applications')}
                                />
                                <NumInput
                                    label="Naukri Applications"
                                    value={form.naukri_applications}
                                    onChange={f('naukri_applications')}
                                />
                                <NumInput
                                    label="Indeed Applications"
                                    value={form.indeed_applications}
                                    onChange={f('indeed_applications')}
                                />
                            </div>
                            ...
                            <div className="flex items-center justify-end gap-3">
                                {saveMsg && (
                                    <span
                                        className={`text-sm font-medium ${saveMsg === 'Saved!' ? 'text-emerald-500' : 'text-rose-500'}`}
                                    >
                                        {saveMsg}
                                    </span>
                                )}
                ...
                <div>
                            ...
                            {filterMode === 'single' && (
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        className="rounded-xl border border-neutral-200 bg-white px-3 py-1.5 text-sm text-neutral-700 shadow-sm focus:ring-2 focus:ring-neutral-900/20 focus:outline-none dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-200"
                                    />
                                    <button
                                        onClick={() => {
                                            setSingleDate(todayStrVal);
                                            setFilterMode('today');
                                        }}
                                        className="text-neutral-400 hover:text-neutral-600"
                                    >
                    ...
                    >
                        ...
                        <span className="text-xs font-medium text-neutral-400 transition group-hover:text-neutral-700 dark:group-hover:text-neutral-200">
                            Tap to log →
                        </span>
                    </button>
                    <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                        ...
                        <StatCard
                            ...
                            value={summary.linkedin_applications}
                            gradient="bg-sky-500"
                        />
                        <StatCard
                            icon={TrendingUp}
                            label="Naukri Applied"
                            value={summary.naukri_applications}
                            gradient="bg-orange-500"
                        />
                        <StatCard
                            icon={TrendingUp}
                            label="Indeed Applied"
                            value={summary.indeed_applications}
                            gradient="bg-indigo-500"
                        />
                    </div>
                    <div className="mt-4 flex flex-wrap gap-3">
                        {profileFields.map(({ key, label, icon }) => {
                            const stat = profileStats.find(
                                (s) => s.key === key,
                            )!;
                            const isRangeMode =
                                filterMode === 'this-week' ||
